    ...
    try:
        while True:
            try:
                ...
                print(f"Received data from client {client_id}: {message.get('type', 'unknown')}")
                
                if message.get("type") == "frame":
                    frame_data = message.get("data", {})
                    print(f"Frame received from {client_id}, deviceId: {frame_data.get('deviceId')}")
...
if __name__ == '__main__':
    ...
    create_insight(
        "System Initialized",
        "Bee monitoring system is now active and collecting data."
    )
